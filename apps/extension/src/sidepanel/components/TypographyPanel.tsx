...
export const TypographyPanel = () => {
  ...
  return (
    <div className="space-y-3">
      ...
      {scan.fonts.length === 0 ? (
        <section className="cs-card p-6 text-center text-xs text-muted">No fonts found</section>
      ) : (
        scan.fonts.map((font, i) => (
          <section key={i} className="cs-card space-y-3 p-4">
            <div>
              <h3
            ...
            <div className="border-t border-line pt-3">
              <div className="cs-heading">Weights</div>
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {font.weights.map((w) => (
                  <span key={w} className="cs-chip">
                    {w}
                  </span>
                ))}
              </div>
            ...
            <div className="border-t border-line pt-3">
              <div className="cs-heading">Sizes</div>
              <div className="mt-1.5 flex flex-wrap gap-1.5">
                {font.sizes.map((s) => (
