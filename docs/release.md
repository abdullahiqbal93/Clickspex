...
| Authentication and pairing      | The bridge listens on `127.0.0.1`, accepts configured Chrome extension origins only, requires a one-time pairing code, and then requires `Authorization: Bearer <token>` for protected requests.                                                 |
